                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($roles as $role)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-4 max-w-xs text-sm text-gray-500 dark:text-gray-300 text-center">
                                    @if(viewContent($project, 'Roles', 'Restore'))
                                        <div class="inline-block">
                                            <form action="{{route('roles.restore',[ 'project'=>$project, 'id'=>$role->id ])}}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                    ...
                                    @endif
                                    @if(viewContent($project, 'Roles', 'ForceDelete'))
                                        <span class="ml-2 mr-2">|</span>
                                        <div class="inline-block">
                                            <form action="{{route('roles.force',[ 'project'=>$project, 'id'=>$role->id ])}}" method="POST" onsubmit="return confirm('Completely remove this Role?');">
                                                @csrf
                                                @method('DELETE')
                                                <button title="Delete">
