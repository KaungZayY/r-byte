                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($backlogs as $backlog)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="px-4 py-4 max-w-xs text-sm text-gray-500 dark:text-gray-300 text-center">
                                    @if (viewContent($project, 'Backlogs', 'Restore'))
                                        <div class="inline-block">
                                            <form action="{{route('backlogs.restore',$backlog->id)}}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                    ...
                                    @endif
                                    @if (viewContent($project, 'Backlogs', 'ForceDelete'))
                                        <span class="ml-2 mr-2">|</span>
                                        <div class="inline-block">
                                            <form action="{{route('backlogs.force',$backlog->id)}}" method="POST" onsubmit="return confirm('Completely remove this backlog?');">
                                                @csrf
                                                @method('DELETE')
                                                <button title="Delete">
