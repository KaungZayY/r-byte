                ...
                <div class="flex space-x-4 items-center">
                    ...
                    @if (viewContent($project, 'Backlogs', 'Create'))
                        <form action="{{ route('backlogs.create', $project) }}" method="GET">
                            <button type="submit" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 dark:bg-green-500 hover:bg-green-700 dark:hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                Create New Backlog
                            </button>
                        </form>
                    @endif
                        ...
                        @foreach ($backlogs as $backlog)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-800">
                                ...
                                <td class="px-4 py-4 max-w-xs text-sm font-medium text-gray-900 dark:text-white text-center uppercase">
                                    ...
                                    @elseif ($backlog->status === 'assigned')
                                ...
                                <td class="px-4 py-4 max-w-xs text-sm text-gray-500 dark:text-gray-300 text-center">
                                    @if (viewContent($project, 'Backlogs', 'Update'))
                                        <div class="inline-block">
                                            <form action="{{route('backlogs.edit',['project' => $project, 'backlog' => $backlog])}}" method="GET">
                                                <button title="Edit">
                                    ...
                                    @endif
                                    @if (viewContent($project, 'Backlogs', 'Delete'))
                                        <span class="ml-2 mr-2">|</span>
                                        <div class="inline-block">
                                            <form action="{{route('backlogs.delete',['project' => $project, 'backlog' => $backlog])}}" method="POST" onsubmit="return confirm('Move this backlog to Archives?');">
                                                @csrf
                                                @method('DELETE')
                                                <button title="Delete">
