        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-3">
                ...
                <div class="flex space-x-4 items-center">
                    ...
                    @if (viewContent($project, 'Teammates', 'Invite'))
                        ...
                        @foreach ($teammates as $teammate)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-800">
                                ...
                                <td class="px-4 py-4 max-w-xs text-sm text-gray-500 dark:text-gray-300 text-center break-words">
                                    {{ $roles->get($teammate->user->id)->role_name ?? '-' }}
                                </td>                                
                                <td class="px-4 py-4 max-w-xs text-sm text-gray-500 dark:text-gray-300 text-center">
                                    <div class="inline-block">
                                        @if (isset($roles->get($teammate->user->id)->id))
                                            @if (viewContent($project, 'Teammates', 'ReassignRole'))
                                                <form action="{{route('roles.reassign',['project'=>$project,'team'=>$team,'user'=>$teammate->user])}}" method="GET">
                                                    <button title="Update Role">
                                                        <svg xmlns="http://www.w3.org/2000/svg" height="22" width="22" viewBox="0 0 640 512">
                                                            <path fill="#f97316" d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3C0 498.7 13.3 512 29.7 512l293.1 0c-3.1-8.8-3.7-18.4-1.4-27.8l15-60.1c2.8-11.3 8.6-21.5 16.8-29.7l40.3-40.3c-32.1-31-75.7-50.1-123.9-50.1l-91.4 0zm435.5-68.3c-15.6-15.6-40.9-15.6-56.6 0l-29.4 29.4 71 71 29.4-29.4c15.6-15.6 15.6-40.9 0-56.6l-14.4-14.4zM375.9 417c-4.1 4.1-7 9.2-8.4 14.9l-15 60.1c-1.4 5.5 .2 11.2 4.2 15.2s9.7 5.6 15.2 4.2l60.1-15c5.6-1.4 10.8-4.3 14.9-8.4L576.1 358.7l-71-71L375.9 417z"/>
                                                        </svg>
                                                    </button>
                                                </form>
                                            @endif
                                        @else
                                            @if (viewContent($project, 'Teammates', 'AssignRole'))
                                                <form action="{{route('roles.assign',['project'=>$project,'team'=>$team,'user'=>$teammate->user])}}" method="GET">
                                                    <button title="Assign Role">
                                                        <svg xmlns="http://www.w3.org/2000/svg" height="22" width="22" viewBox="0 0 640 512">
                                    ...
                                    @if (viewContent($project, 'Teammates', 'Delete'))
                                        <span class="ml-2 mr-2">|</span>
                                        <div class="inline-block">
                                            <form action="{{route('teammates.delete',$teammate)}}" method="POST" onsubmit="return confirm('Remove this member from team?');">
                                                @csrf
                                                @method('DELETE')
                                                <button title="Remove">
                                                    <svg xmlns="http://www.w3.org/2000/svg" height="22" width="22" viewBox="0 0 640 512">
